...
SELECT F.FILM_ID,F.TITLE AS FILM_NAME,C.NAME AS CAT_NAME,LANG.NAME AS LANG_NAME
...
SELECT P.CUSTOMER_ID,SUM(AMOUNT) AS SPENDING, C.FIRST_NAME, C.LAST_NAME, C.EMAIL
...
SELECT S.STORE_ID, SUM(P.AMOUNT) AS STORE_REVENUE_PER_STORE
...
SELECT monthname(RENTAL_DATE) AS MONTH_NAME, EXTRACT(YEAR FROM RENTAL_DATE) AS YEAR, COUNT(RENTAL_ID) AS RENTALS
FROM RENTAL
